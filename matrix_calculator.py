        ...
        def add_matrices():
            ...
            for i in range(row1):
                for j in range(col1):
                    ...
                    label=tk.Label(result_frame,text=str(value),width=12,font=('Arial', 16),relief="solid")
                    label.grid(row=i,column=j,padx=2,pady=2)

        evaluate_btn=tk.Button(set_matrix_add_win_mid_frame,text="EVALUATE",command=add_matrices).grid(row=start_row_C,column=0,columnspan=6)
...
addition_matrix_button=tk.Button(matrix_frame,text="Addition_Matrix",width=20,height=5,command=set_matrix_for_addition_matrix)
addition_matrix_button.grid(row=0,column=0,columnspan=5)


subtract_matrix_button=tk.Button(matrix_frame,text="Substraction_Matrix",width=20,height=5)
